...
export function CartDrawer() {
  const { cartItems, isCartOpen, setIsCartOpen, removeFromCart, updateQuantity, cartTotal } = useStorefrontCart();
            ...
            <div className="flex items-center justify-between border-b border-gray-100 p-6">
              <h2 className="flex items-center text-xl font-serif uppercase tracking-widest text-bg-primary">
                {step === "cart" ? (
                  ...
                  <>
                    <ArrowLeft size={20} className="mr-3 cursor-pointer text-accent-gold" onClick={() => setStep("cart")} />
                    Order Details
                  </>
                )}
              </h2>
              <button onClick={() => setIsCartOpen(false)} className="text-bg-primary transition-colors hover:text-accent-gold">
                <X size={24} />
              </button>
            </div>
              ...
              {step === "cart" ? (
                cartItems.length > 0 ? (
                      ...
                      <div key={item.id} className="group flex space-x-4">
                        ...
                        <div className="flex-grow">
                          <div className="mb-1 flex items-start justify-between">
                            <h3 className="font-serif text-sm uppercase tracking-wide text-bg-primary">{item.name}</h3>
                            <button onClick={() => removeFromCart(item.id)} className="text-gray-300 transition-colors hover:text-red-500">
                              <X size={16} />
                            </button>
                          </div>
                          <p className="mb-4 text-xs uppercase tracking-widest text-accent-bronze">{item.category}</p>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center rounded-sm border border-gray-100">
                              <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1 px-2 text-bg-primary hover:bg-gray-50">
                                <Minus size={12} />
                              </button>
                            ...
                            <p className="text-sm font-bold text-accent-gold">${(item.price * item.quantity).toLocaleString()}</p>
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="flex h-full flex-col items-center justify-center text-center">
                    ...
                    <h3 className="mb-2 text-xl font-serif text-bg-primary">Your cart is empty</h3>
                    <p className="mb-8 text-sm text-gray-400">Discover our collection and start your story.</p>
                    <button onClick={() => setIsCartOpen(false)} className="border-b-2 border-accent-gold pb-1 text-xs font-bold uppercase tracking-widest text-accent-gold">
                      Browse Collection
                    </button>
                  </div>
                )
                ...
                <form id="order-form" onSubmit={handlePlaceOrder} className="space-y-6">
                  {[
                    ...
                    { name: "phone", label: "Phone Number", type: "tel", placeholder: "e.g. +1 234 567 890" },
                  ].map((field) => (
                    <div key={field.name}>
                      <label className="mb-2 block text-[10px] font-bold uppercase tracking-[0.2em] text-bg-primary/40">{field.label}</label>
                      <input
                        required
                        type={field.type}
                        name={field.name}
                  ...
                  <div>
                    <label className="mb-2 block text-[10px] font-bold uppercase tracking-[0.2em] text-bg-primary/40">Shipping Address</label>
                    <textarea
                      required
                      name="address"
                      rows={3}
                  ...
                  <div>
                    <label className="mb-2 block text-[10px] font-bold uppercase tracking-[0.2em] text-bg-primary/40">Additional Notes (Optional)</label>
                    <textarea
                      name="notes"
                      rows={2}
