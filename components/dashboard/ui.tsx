...
export function Panel({
  ...
  return (
    <div className={cn("glass-panel gold-ring rounded-[1.5rem] border border-[var(--border)]", className)}>
      {children}
    </div>
  );
}
...
export function Button({
  ...
  const variants = {
    ...
    secondary: "border border-[var(--border)] bg-[var(--surface-2)] text-[var(--text)] hover:border-[var(--gold)]",
...
export function PageHeader({
  ...
  return (
    <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        <h1 className="font-[family-name:var(--font-display)] text-5xl text-[var(--text)]">{title}</h1>
        <p className="mt-2 max-w-2xl text-sm text-[var(--text-dim)]">{description}</p>
      </div>
      {action}
    </div>
  );
...
export function EmptyState({
  ...
  return (
    <Panel className="flex flex-col items-center px-6 py-16 text-center">
      ...
      <h2 className="mt-5 font-[family-name:var(--font-display)] text-4xl text-[var(--text)]">{title}</h2>
  ...
  return (
    <Panel className="p-6">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.28em] text-[var(--text-muted)]">{label}</p>
          <p className="mt-4 text-4xl font-semibold text-[var(--gold)]">{value}</p>
        </div>
...
export function SkeletonBlock({ className }: { className?: string }) {
  return <div className={cn("shimmer rounded-2xl bg-[var(--surface-2)]", className)} />;
}
